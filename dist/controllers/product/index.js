...
const Create = async (req, res, next) => {
  ...
  try {
    ...
    const lastProduct = await _product2.default.findOne({ gender })
      .sort({ myid: -1 })
      .select('myid');
...
const Update = async (req, res, next) => {
  ...
  try {
      ...
      const existingProduct = await _product2.default.findById(product_id);
      if (!existingProduct) {
          return next(Boom.notFound("Product not found."));
      }
      ...
      const updated = await _product2.default.findByIdAndUpdate(product_id, updatedProduct, {
          new: true,
...
const updateSale = async (req, res, next) => {
  ...
  try {
    ...
    const products = await _product2.default.find({ _id: { $in: objectIds } });
    ...
    const updated = await _product2.default.bulkWrite(
...
const GetCompleteList = async (req, res, next) => {
  try {
    const products = await _product2.default.find({}).sort({ createdAt: -1 });
    res.json(products);
  } catch (e) {
    next(e);
  }
...
const GetTotalCount = async (req, res, next) => {
  try {
    const totalCount = await _product2.default.countDocuments();
    res.json({ totalCount });
  } catch (e) {
    next(e);
  }
...
const fetchProductsByIds = async (req, res) => {
  ...
  try {
    ...
    const products = await _product2.default.find({ _id: { $in: ids } });
...
const fetchProductsByMen = async (req, res) => {
  try {
    ...
    const products = await _product2.default.aggregate([
